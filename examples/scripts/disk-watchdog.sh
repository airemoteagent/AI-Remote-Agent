...
#
# A mona-agent recipe: run this from cron (or the dashboard's cron tasks)
# so the agent checks disk health and notifies you before things break.
#
#   crontab -e
#   # every morning at 8:00
#   0 8 * * * /path/to/mona-agent/examples/scripts/disk-watchdog.sh
#
...
THRESHOLD="${MONA_WATCHDOG_THRESHOLD:-85}"

if ! command -v mona-agent >/dev/null 2>&1; then
  echo "disk-watchdog: mona-agent not found on PATH" >&2
  exit 1
fi
...
mona-agent chat "Run the disk-health skill: check all volumes. Flag every volume above ${THRESHOLD}% with what it is, how much is free, and one safe cleanup suggestion (propose only, never delete)."
